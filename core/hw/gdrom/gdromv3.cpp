...
void gd_setdisc()
{
	...
	if (gd_disk_type == DiscType::Busy && newd != DiscType::Busy)
	{
		GDStatus.BSY = 0;
		GDStatus.DRDY = 1;
	}
	...
	SecNumber.DiscFormat = (int)gd_disk_type >> 4;
}
void gd_reset()
{
	//Reset the drive
...
#define readcmd packet_cmd.GDReadBlock
	...
	case 0x71:
	{
		...
		if ((DiscType)libGDR_GetDiscType() == DiscType::GdRom || (DiscType)libGDR_GetDiscType() == DiscType::CdRom_XA)
			SecNumber.Status = GD_PAUSE;
		else
			SecNumber.Status = GD_STANDBY;
	}
