...
bool CHDDisc::TryOpen(const wchar* file)
{
	...
	for (;;)
	{
		...
		t.file = std::make_unique<CHDTrack>(this, t.StartFAD, total_hunks, strcmp(type, "MODE1") ? 2352 : 2048);
...
std::unique_ptr<Disc> chd_parse(const std::string& file)
{
	auto chdDisc = std::make_unique<CHDDisc>();
	
	if (chdDisc->TryOpen(file.c_str()))
		return std::move(chdDisc);
	return nullptr;
}