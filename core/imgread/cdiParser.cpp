...
std::unique_ptr<Disc> cdi_parse(const std::string& file)
{
	core_file* fsource = core_fopen(file.c_str());

	if (!fsource)
		return 0;

	auto rv = std::make_unique<Disc>();
	...
	while (image.remaining_sessions > 0)
	{
		...
		else
		{
			...
			while (image.remaining_tracks > 0)
			{
				...
				t.file = new RawTrackFile(core_fopen(file.c_str()), track.position + track.pregap_length * track.sector_size, t.StartFAD, track.sector_size);
