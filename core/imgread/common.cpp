...
bool InitDrive_(const wstring& fn)
{
	TermDrive();

	//try all drivers
	std::string fileNameAnsi;
	fileNameAnsi.resize(fn.length());
//	//errno_t wcstombs_s(
//   size_t *pReturnValue,
//   char *mbstr,
//   size_t sizeInBytes,
...
//);
	...
	::wcstombs_s(&bytesConverted, &fileNameAnsi[0], fn.size(), &fn[0], fn.size());
	disc = OpenDisc(fileNameAnsi.c_str());
...
bool InitDrive(u32 fileflags)
{
	...
	std::wstring diskImageName = settings.imgread.LastImage;

#ifdef BUILD_DREAMCAST
	if (diskImageName == L"null")
	{
		const auto result = GetDiskImageFileName();
		if (!result.second)
		{
		...
		// FIXME: Data loss if buffer is too small
		settings.imgread.LastImage = diskImageName;

		SaveSettings();

		if (!InitDrive_(diskImageName.c_str()))
		{
...
bool DiscSwap(u32 fileflags)
{
	...
	// FIXME: Data loss if buffer is too small
	std::wstring fn = settings.imgread.LastImage;
	
#ifdef BUILD_DREAMCAST
	const auto gfrv = GetDiskImageFileName();
#else
	int gfrv = 0;
#endif
	if (!gfrv.second)
